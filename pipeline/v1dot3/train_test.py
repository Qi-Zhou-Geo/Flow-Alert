...
# using ".parent" on train_test_2017-2020_9repeat "pathlib.Path" object moves one level up the directory hierarchy
...
from functions.data_process.archive_data import dump_evaluate_matrix, dump_evaluate_matrix_buffer
from functions.data_process.archive_data import dump_model_prediction, dump_as_row
...
class Train_Test:
    ...
    def __init__(self,
        ...
        self.test_monitor = -1e3  # monitor test F1
        # </editor-fold>
        ...
        self.loss_func = torch.nn.CrossEntropyLoss(reduction="mean", weight=class_weight_binary, label_smoothing=0.05)
        # </editor-fold>
    ...
    def training(self, epoch, dump_model=False, purpose="training"):
        ...
        for dataloader in self.train_dataloader:
            ...
            for batch_data in dataloader:
                ...
                if total_norm > 10:
                    print(f"Clipped from {total_norm:.2f} to {self.max_norm_clip}, loss={loss.item()}")

                self.optimizer.step()

                # predicted probability of debris flow label 1
                record = prepare_records(t_target, target, raw_logits, DF_threshold=self.DF_threshold)
    ...
    def testing(self, epoch, purpose, received_dataloader, received_model=None):

        if received_model is None:
            model_test = self.model
        else:
            model_test = received_model
        model_test.eval()
        ...
        with torch.no_grad():
            ...
            for idx, dataloader in enumerate(received_dataloader):
                ...
                for batch_data in dataloader:
                    ...
                    raw_logits = model_test(features, t_features)  # return the model output logits, shape (batch_size, 2)
                    loss = self.loss_func(raw_logits, target)
                    epoch_loss += loss.item()
                ...
                # make sure the first column is time stamps
                tensor_temp = tensor_temp.detach().cpu().numpy()
                be_saved_array = tensor_temp[tensor_temp[:, 0].argsort()]

                # dump the results
                # use the unbuffered confusion matrix
                # evaluate_matrix = dump_evaluate_matrix(be_saved_array,
                # f"{purpose}, epoch_{epoch}_{self.output_format}",
                # f"{self.output_dir}", "summary_LSTM_all")

                # use the buffered confusion matrix, check the function for details
                evaluate_matrix = dump_evaluate_matrix_buffer(be_saved_array,
                                                              f"{purpose}, epoch_{epoch}_{self.output_format}",
                                                              f"{self.output_dir}", "summary_LSTM_all")
    ...
    def activation(self, num_epoch=50, testing=True):
        ...
        for epoch in range(1, num_epoch + 1):  # loop 50 times for training
            ...
            if test_matrix[-2] > self.test_monitor:
                ...
                train_matrix_temp = f"Training, epoch_{epoch}_{self.output_format}, {', '.join(map(str, train_matrix))}"
                test_matrix_temp = f"testing, epoch_{epoch}_{self.output_format}, {', '.join(map(str, test_matrix))} \n"
                _save_checkpoint(self.model, self.output_dir, f"{self.output_format}", epoch)

            # record the logs
            time_now = UTCDateTime().isoformat()
            record = (f"epoch = {epoch}/{num_epoch}, UTC+0 = {time_now}, \n"
                      f"train_loss = {train_loss}, test_loss = {test_loss}, \n"
                      f"self.train_monitor (epoch loss) = {self.train_monitor}, self.test_monitor (F1) = {self.test_monitor}, \n"
                      f"test_matrix = {test_matrix} \n")
