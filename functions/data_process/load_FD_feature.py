...
## for seismic feature
def load_FD_features(catchment_name, seismic_network, input_year, input_station, input_component,
    ...
    # FD sets
    df0 = pd.read_csv(f"{feature_dir}/{input_year}_{input_station}_{input_component}_all_FD.txt",
    ...
    df = pd.concat([df0, df1, df2, df4], axis=1, ignore_index=True)
    columnsName = np.concatenate([df0.columns.values, df1.columns.values, df2.columns.values,  df4.columns.values])
...
def select_features(catchment_name, seismic_network, input_year, input_station, input_component, feature_type, with_label,
    ...
    if feature_type == "A":  # BL sets
        ...
        selected_column.extend([80, 81, 82])
        with_network = True
    elif feature_type.split("-")[0] == "FD":
        selected_column = np.arange(0, 76).tolist()
        with_network = False
    else:
        print(f"please check the {feature_type}")


    # if you want to normalize the feature, run functions/data_process/min_max_normalize_fit.py
    df = load_FD_features(catchment_name, seismic_network, input_year, input_station, input_component,
