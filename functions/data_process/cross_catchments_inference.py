...
def _dump_results(array_temp, models, output_format, output_path):
    column_name = ["t_target", "t_str", "label"] + [f"pro{i}" for i in range(len(models))] + ["pro_mean", "ci_range"]
...
def load_feature_as_sequences(st, sub_window_size, window_overlap, feature_type, seq_length, synthetic_length):
    ...
    output_feature = stream_to_feature.prepare_feature(st=st, normalize_type="ref-training")
...
def plot_predicted_pro(benchmark_time, first_detection_str, st, array_temp, output_path, output_format, note):
    ...
    for t, l in zip(times, labels):
        delta_t = UTCDateTime(t) - UTCDateTime(tr.stats.starttime)
        ax.axvline(x=delta_t * sps, label=l, zorder=1)
