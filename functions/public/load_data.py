...
def config_catchment_path(catchment_name, seismic_network):
    ...
    current_dir = Path(__file__).resolve().parent
    project_root = current_dir.parent.parent
    config_path = (project_root / f"./config/config_catchment_code.yaml").resolve()
...
def load_all_features(catchment_name, seismic_network, input_year, input_station, input_component,
    ...
    feature_dir = f"{CONFIG_dir['feature_output_dir']}/" \
    ...
    df1 = pd.read_csv(f"{feature_dir}/{input_year}_{input_station}_{input_component}_all_A.txt",
                      ...
                      usecols=[4, 5, 6, 7, 8, 9, 10, 11, 12, 14, 17]) # 14 is 'goodness', 16 is 'magnitude_range'
    ...
    if with_network is True:  # need the network feature
        network_feature_dir = f"{CONFIG_dir['feature_output_dir']}/" \
                              f"{config_catchment_path(catchment_name, seismic_network)}/" \
    ...
    else:  # do NOT need the network feature, and generate synthetic data to keep the df4 structure
...
def select_features(catchment_name, seismic_network, input_year, input_station, input_component, feature_type, with_label,
    ...
    Args:
        ...
        input_year: str, year of data, e.g., "2014"
        input_station: str, station of data, e.g., "ILL12"
        input_component: str, component of data, e.g., "EHZ"
    ...
    Returns:
        ...
        df: data frame, column by [time_stamp_float, x, y]
    '''
    ...
    if feature_type == "A":  # BL sets
        ...
        #import yaml
        #with open("./config/config_train_test.yaml", "r") as f:
            #config = yaml.safe_load(f)
        selected_column = [22, 23, 24, 25, 35]#config["feature_type_E"] # -> [22, 23, 24, 25, 35]
        selected_column.extend([80, 81, 82])
        with_network = False
...
## for waveform
def load_waveform(catchment_name, seismic_network, input_year, input_station, input_component):

    id1, id2 = data_length_mapping(seismic_network, input_year, input_station, input_component)

    f1 = [1, 5,  10, 15, 20] # lower frequency boundary
    f2 = [5, 10, 15, 20, 25] # upper frequency boundary

    file_dir = f"{CONFIG_dir['feature_output_dir']}/" \
               f"{config_catchment_path(catchment_name, seismic_network)}/" \
               f"{input_year}/{input_station}/{input_component}"
    ...
    for idx, julday_id in enumerate(np.arange(id1, id2+1)):
        file_name = f"{seismic_network}-{input_station}-{input_component}-{input_year}-{str(int(julday_id)).zfill(3)}"

        # add as column
        temp_array = None
        for step in np.arange(len(f1)):
            dataset_name = f"{file_name}-{f1[step]}-{f2[step]}"
            # data is 2D array (time stamps, amp in m/s)
            data, metadata = load_hdf5(file_dir, file_name, dataset_name)
            if step == 0:
                temp_array = data
            else:
                temp_array = np.concatenate((temp_array, data), axis=1) # axis=1 as column

        # add as row
        if idx == 0:
            amp_array = temp_array
        else:
            amp_array = np.concatenate((amp_array, temp_array), axis=0)  # axis=0 as row

    select = np.arange(1, len(f1)*2, 2).tolist()
    select.insert(0, 0) # insert 0 in the first place
    # row by [t1, t2, ..., ]
    # column by [time_stamps, amp1, amp2, ..., ]
    amp_array = amp_array[:, select]
...
def load_waveform_pro(amp_array, seismic_network, input_year,
    ...
    # if noone manually label the data, set
    ...
    file_dir = f"{CONFIG_dir['parent_dir']}"
    df = pd.read_csv(f"{file_dir}/config/manually_labeled_DF/{seismic_network}-{input_year}-DF.txt", header=0)
