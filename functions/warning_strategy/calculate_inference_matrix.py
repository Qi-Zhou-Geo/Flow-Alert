...
def inference_matrix(array_temp, benchmark_time, event_start, event_end, pro_epsilon=0.9, buffer1=0.5, buffer2=1):
    '''
    Calculation of the evaluation matrix

    Args:
        benchmark_time: str, format in "%Y-%m-%dT%H:%M:%S"
        pro_st_event: Obspy Stream or Trace, model predicted probability during the event
        pro_st_all: Obspy Stream or Trace, model predicted probability for the entire event
        pro_epsilon: float, the threshold to trigger warning
        fmt: str, the time format

    Returns:
        phi: float, 1 denotes the event detected or warned
        psi: float, false positive rate,
        delta_t: float, the time between 1st warning and benchmark time
        warning_time_float: float, time stamps in UTC
        warning_time_str: str, time string
    '''
    ...
    id_s = np.argmin(temp_t1)  # unit is second
    if id_s >= 600:
        print(f"Warning!\n"
              f"The event buffer time may have error.\n"
              f"benchmark_time={benchmark_time}, id_s={t_str[id_s]}\n")

    id_e = np.argmin(temp_t2)
    if id_s >= 600:
        print(f"Warning!\n"
              f"The event buffer time may have error.\n"
              f"benchmark_time={benchmark_time}, id_e={t_str[id_e]}\n")
